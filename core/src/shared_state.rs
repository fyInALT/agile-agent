...
impl LoopControlFlags {
    ...
    pub fn start_loop(&mut self, max_iterations: usize) {
        logging::debug_event("loop.start", "autonomous loop started",
            serde_json::json!({"max_iterations": max_iterations, "previous_active": self.loop_run_active}));
    ...
    pub fn stop_loop(&mut self) {
        logging::debug_event("loop.stop", "autonomous loop stopped",
            serde_json::json!({"current_iteration": self.current_iteration, "was_active": self.loop_run_active}));
    ...
    pub fn increment_iteration(&mut self) {
        ...
        logging::debug_event("loop.iteration", "iteration completed",
            serde_json::json!({"current_iteration": self.current_iteration, "max_iterations": self.max_iterations, "remaining": self.remaining_iterations()}));
    }

    /// Signal quit to all agents
    pub fn signal_quit(&mut self) {
        logging::debug_event("loop.quit", "quit signal received",
            serde_json::json!({"current_iteration": self.current_iteration, "loop_run_active": self.loop_run_active, "loop_paused": self.loop_paused}));
        self.should_quit = true;
    }

    /// Pause loop execution
    pub fn pause(&mut self) {
        logging::debug_event("loop.pause", "loop execution paused",
            serde_json::json!({"current_iteration": self.current_iteration}));
        self.loop_paused = true;
    }

    /// Resume loop execution
    pub fn resume(&mut self) {
        logging::debug_event("loop.resume", "loop execution resumed",
            serde_json::json!({"current_iteration": self.current_iteration}));
